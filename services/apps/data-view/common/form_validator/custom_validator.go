...
var customerValidators = []*struct {
	...
	{
		tag:           "VerifyName50d",
		validatorFunc: VerifyName50d,
		trans: map[string]string{
			"zh": "{0}长度必须不超过50，仅支持中英文、数字、下划线",
			"en": "{0}长度必须不超过50，仅支持中英文、数字、下划线",
		},
	},
	{
	...
	{
		tag:           "VerifyTechnicalName",
		validatorFunc: VerifyTechnicalName,
		trans: map[string]string{
			"zh": "{0}长度必须不超过100，只能包含小写字母，数字和下划线，且不能以数字开头",
			"en": "{0}长度必须不超过100，只能包含小写字母，数字和下划线，且不能以数字开头",
		},
	},
	{
	...
	{
		tag:           "VerifyMultiDataSourceType",
		validatorFunc: VerifyMultiDataSourceType,
		trans: map[string]string{
			"zh": "{0}数据源类型必须是  mysql maria  postgresql sqlserver  oracle hive-hadoop2 中的一个或多个，多个用逗号分隔",
			"en": "{0}数据源类型必须是  mysql maria  postgresql sqlserver  oracle hive-hadoop2 中的一个或多个，多个用逗号分隔",
		},
	},
	{
		tag:           "VerifyVertexID",
	...
	{
		tag:           "VerifyMultiTaskStatus",
		validatorFunc: VerifyMultiTaskStatus,
		trans: map[string]string{
			"zh": "{0}任务状态必须是 queuing running finished canceled failed中的一个或多个，多个用逗号分隔",
			"en": "{0}任务状态必须是 queuing running finished canceled failed中的一个或多个，多个用逗号分隔",
		},
	},
	{
		tag:           "DataTypeChar",
		validatorFunc: DataTypeChar,
		trans: map[string]string{
			"zh": "{0}格式不正确",
			"en": "{0}格式不正确",
		},
...
// VerifyName50d Must Have
func VerifyName50d(fl validator.FieldLevel) bool {
	f := fl.Field().String()
	f = strings.TrimSpace(f)
	fl.Field().SetString(f)
	...
	compile := regexp.MustCompile("^[a-zA-Z0-9\u4e00-\u9fa5_]+$")
	return compile.Match([]byte(f))
}
...
func VerifyTechnicalName(fl validator.FieldLevel) bool {
	// must have
	f := fl.Field().String()
	f = strings.TrimSpace(f)
	fl.Field().SetString(f)
...
func VerifyMultiDataSourceType(fl validator.FieldLevel) bool {
	...
	for i := 0; i < len(arr); i++ {
		arr[i] = strings.TrimSpace(arr[i])
		if !enum.Is[constant.DataSourceType](arr[i]) {
			return false
		}
	}
	f = strings.Join(arr, ",")
...
func VerifyMultiTaskStatus(fl validator.FieldLevel) bool {
	f := fl.Field().String()
	f = strings.TrimSpace(f)
	fl.Field().SetString(f)
	if f == "" {
	...
	for i := 0; i < len(arr); i++ {
		arr[i] = strings.TrimSpace(arr[i])
		if !enum.Is[explore_task2.TaskStatus](arr[i]) {
			return false
		}
	}
	f = strings.Join(arr, ",")
...
func DataTypeChar(fl validator.FieldLevel) bool {
	f := fl.Field().String()
	f = strings.TrimSpace(f)
	fl.Field().SetString(f)
	...
	//compile := regexp.MustCompile("^[!-~a-zA-Z0-9-_]+$")
	compile := regexp.MustCompile("^[a-zA-Z]{3,}$")
	return compile.Match([]byte(f))
}
